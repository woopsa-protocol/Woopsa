...
namespace Woopsa
{
    public class WoopsaClientObject : WoopsaObject
    {
        ...
        public int Subscribe(string path, PropertyChanged propertyChangedHandler, TimeSpan monitorInterval, TimeSpan publishInterval)
        {
            ...
            while (rootObject.Container != null)
            {
                if (rootObject.Container is WoopsaClientObject)
                    rootObject = rootObject.Container as WoopsaClientObject;
            }
            ...
            if (_subscriptionChannel == null)
            {
                ...
                {
                    _hasSubscriptionService = rootObject.Items.ByNameOrNull(WoopsaServiceSubscriptionConst.WoopsaServiceSubscriptionName) != null;
                }
        ...
        {
            //TODO : Async !
            return _client.ReadAsync(this.GetPath(_root).TrimEnd(WoopsaConst.WoopsaPathSeparator) + WoopsaConst.WoopsaPathSeparator + (sender as IWoopsaProperty).Name).Result;
        }

        private void SetProperty(object sender, IWoopsaValue value)
        {
            //TODO : Async !
            _client.WriteAsync(this.GetPath(_root).TrimEnd(WoopsaConst.WoopsaPathSeparator) + WoopsaConst.WoopsaPathSeparator + (sender as IWoopsaProperty).Name, value.AsText).Wait();
        }
        ...
        private  WoopsaValue Invoke(IEnumerable<IWoopsaValue> arguments, List<WoopsaMethodArgumentInfo> argumentInfos, string methodName)
        {
            ...
            return _client.Invoke(this.GetPath(_root).TrimEnd(WoopsaConst.WoopsaPathSeparator) + WoopsaConst.WoopsaPathSeparator + methodName, namedArguments).Result;
        }
