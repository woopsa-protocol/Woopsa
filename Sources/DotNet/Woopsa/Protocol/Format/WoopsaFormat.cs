...
namespace Woopsa
{
    public static class WoopsaFormat
    {
        ...
        public const string MultipleRequestResponseFormat = "{{\"" + KeyId + "\":{0}, \"" + KeyResult + "\":{1}}}";

        public const string MultipleElementsFormat = "[{0}]";
        ...
        private static Dictionary<WoopsaVerb, string> _dictionnaryVerbToString =
            new Dictionary<WoopsaVerb, string>()
            {
                {WoopsaVerb.Read, VerbRead },
                {WoopsaVerb.Write, VerbWrite },
                {WoopsaVerb.Invoke, VerbInvoke },
                {WoopsaVerb.Meta, VerbMeta }
            };

        private static Dictionary<string, WoopsaVerb> _dictionnaryStringToVerb =
            new Dictionary<string, WoopsaVerb>()
            {
                {VerbRead, WoopsaVerb.Read },
                {VerbWrite, WoopsaVerb.Write },
                {VerbInvoke, WoopsaVerb.Invoke },
                {VerbMeta, WoopsaVerb.Meta }
            };

        public static WoopsaVerb? StringToVerb(string text)
        {
            if (_dictionnaryStringToVerb.ContainsKey(text))
            {
                return _dictionnaryStringToVerb[text];
            }
            return null;
        }

        public static string VerbToString(this WoopsaVerb verb)
        {
            return _dictionnaryVerbToString[verb];
        }
        ...
        {
#if UNITY_WSA
            var error = UnityJsonSerializer.Deserialize<WoopsaErrorResult>(jsonErrorText);
#else
        ...
        {
#if UNITY_WSA
            return UnityJsonSerializer.Deserialize<WoopsaMetaResult>(jsonText);
#else
        ...
        public static WoopsaValue DeserializeWoopsaValue(string jsonText)
        {
            ...
            var result = UnityJsonSerializer.Deserialize<WoopsaReadResult>(jsonText, new WoopsaReadResultConverter());
#else
        ...
        private class WoopsaReadResultConverter : JsonConverter
        {
