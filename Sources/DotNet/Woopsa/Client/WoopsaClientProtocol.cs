...
namespace Woopsa
{
    ...
    public class WoopsaClientProtocol : IDisposable
    {
        ...
        private string Request(string path, NameValueCollection postData, TimeSpan timeout)
        {
            try
            {
                // Note : Use Task.Run to avoid external (HMI for e.g.) SynchronizationContext to flow into woopsa lib.
                return Task.Run(() => RequestAsync(path, postData, timeout)).Result;
            }
